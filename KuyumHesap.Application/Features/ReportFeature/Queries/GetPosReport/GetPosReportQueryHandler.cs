...
namespace KuyumHesap.Application.Features.ReportFeature.Queries.GetPosReport
{
    public class GetPosReportQueryHandler : BaseHandler, IRequestHandler<GetPosReportQueryRequest, ResponseDto<GetPosReportQueryResponse>>
    {
        ...
        public GetPosReportQueryHandler(IMapper mapper, IUnitOfWork unitOfWork, IAccountStatementQuery accountStatementQuery) : base(mapper, unitOfWork)
        {
        ...
        public async Task<ResponseDto<GetPosReportQueryResponse>> Handle(GetPosReportQueryRequest request, CancellationToken cancellationToken)
        {
            ...
            var ekstre = new GetPosReportQueryResponse();
            var responseItem = new List<GetPosReportItemResponse>();
            var items = new GetPosReportItemResponse();
            ...
            var accountByCach = await unitOfWork.GetReadRepository<Account>().GetAllAsync(x => x.AccountTypeId == 6 && !x.IsDeleted);
            ...
            return new ResponseDto<GetPosReportQueryResponse>().Success(ekstre);
        }
